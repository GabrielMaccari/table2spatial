...
from PyQt6.QtWidgets import (QMainWindow, QApplication, QLabel, QCheckBox,
                             ...
                             QScrollArea, QGroupBox, QGridLayout, QInputDialog)
...
class table2spatialApp(QMainWindow):
    def __init__(self):
        ...
        self.CRS_type_cmb.currentTextChanged.connect(
            lambda: self.get_CRS_list(self.CRS_type_cmb.currentText()))
    ...
    def open_file(self):
        try:
            self.inFile = QFileDialog.getOpenFileName(self,
                    caption='Selecione uma tabela contendo dados de entrada.',
                    directory=self.folder,
                    filter='Formatos suportados (*.xlsx *.xlsm *.csv *.ods);;'
                           'Pasta de Trabalho do Excel (*.xlsx);;Pasta de '
                           'Trabalho Habilitada para Macro do Excel (*.xlsm);;'
                           'CSV (*.csv);; OpenDocument Spreadsheet (*.ods)'
                    )
            ...
            try:
                if self.inFile[0].endswith('.csv'):
                    try:
                        ...
                        msg = QMessageBox(
                            parent=self,
                            text='Você selecionou um arquivo de texto separado '
                                 'por delimitador (CSV). Para um funcionamento '
                                 'adequado da ferramenta, certifique-se de que '
                                 'o separador de campo utilizado no arquivo não'
                                 ' está presente no conteúdo das células e não '
                                 'coincide com o separador decimal.'
                            )
                        ...
                        msg = QMessageBox(
                            parent=self,
                            text=f'Não foi possível realizar a leitura do '
                                 'arquivo CSV. Certifique-se de que o separador'
                                 ' de campo utilizado no arquivo não está '
                                 'presente no conteúdo das células e não '
                                 'coincide com o separador decimal.\n\n{str(e)}'
                            )
                    ...
                    eng = ('odf' if self.inFile[0].endswith('.ods')
                           else 'openpyxl')
                    ...
                    if len(file.sheet_names) > 1:
                        ...
                        msg = QMessageBox(
                            parent=self,
                            text='O arquivo selecionado contém múltiplas abas. '
                                 'Caso deseje mesclar todas as abas em uma '
                                 'única planilha, marque a caixa "Mesclar abas '
                                 'com a coluna" na interface da ferramenta e '
                                 'selecione uma coluna em comum para realizar a'
                                 ' mesclagem.'
                            )
                    ...
                    remove_cols = [col for col in self.DF.columns if
                                   'Unnamed' in col]
                ...
                msg = QMessageBox(parent=self,
                                  text='Não foi possível abrir o arquivo.'
                                       f'\n\n{str(e)}'
                                 )
                msg.setWindowTitle('Erro')
                msg.setIcon(QMessageBox.Icon.Critical)
    ...
    def merge_sheets(self):
        ...
        if not allSheets:
            msg = QMessageBox(parent=self,
                              text='As seguintes abas não possuem a coluna '
                                   f'"{self.mergeColumn}", e portanto não serão'
                                   ' adicionadas ao dataframe:'
                                   f'\n{str(sheets_to_ignore)}'
                              )
            msg.setWindowTitle('Atenção')
            msg.setIcon(QMessageBox.Icon.Warning)
            msg.exec()
        ...
        try:
            # Lê todas as abas do arquivo e armazena em uma lista. Armazena
            # também os dtypes das colunas de merge de cada aba
            sheets, merge_dts = [], []
            for i in range(0, n):
            ...
            # Mescla todas as abas com base na coluna de mescla selecionada pelo
            # usuário
            df = sheets[0]
            for i in range(1, len(sheets)):
                df = pandas.merge(df, sheets[i], how='outer',
                                  on=self.mergeColumn)
            ...
            msg = QMessageBox(parent=self,
                              text=f'Não foi possível mesclar as abas com a '
                                   f'coluna especificada.\n\n{str(e)}')
            msg.setWindowTitle('Erro')
            msg.setIcon(QMessageBox.Icon.Critical)
            msg.exec()
        ...
        for s in range(len(sections)):
            if s in to_enable:
                for widget in sections[s]:
                    if widget == self.mergeColumn_cmb \
                            or widget == self.merge_btn:
    ...
    def update_columns_list(self):
        ...
        for i in range(len(self.columns)):
            ...
            combo.setCurrentText(list(self.dTypeDict.keys())[
                                     list(self.dTypeDict.values()).index(dt)])
            combo.currentTextChanged.connect(
                lambda wontWorkWhitoutThis, x=i: self.switch_dType(x))
            status = QLabel('OK')
    ...
    def switch_dType(self, i, update_NS_columns=True):
        ...
        try:
            ...
            msg = QMessageBox(parent=self,
                              text=f'Os dados da coluna "{self.columns[i]}" não'
                                   f' podem ser convertidos para o formato '
                                   f'{self.dTypeCombos[i].currentText()}.\n\n'
                                   f'Motivo: {str(e)}')
            msg.setWindowTitle('Atenção')
            msg.setIcon(QMessageBox.Icon.Warning)
            msg.exec()
    ...
    def update_coordinates_columns(self):
        ...
        for i in range(len(self.columns)):
            if self.dTypes[i] in allowed_dTypes:
                c = self.columns[i]
                if self.DF[c].dropna().between(y_range_min,
                                               y_range_max).all() and not \
                self.DF[c].dropna().empty:
                    y_columns_list.append(c)
                if self.DF[c].dropna().between(x_range_min,
                                               x_range_max).all() and not \
                self.DF[c].dropna().empty:
    ...
    def export_file(self):
        ...
        if self.N_column == '' or self.E_column == '':
            ...
            file_options = 'Geopackage (pontos) (*.gpkg);;ESRI Shapefile ' \
                           '(pontos) (*.shp);;GeoJSON (*.json);; Pasta de ' \
                           'Trabalho do Excel (*.xlsx);;CSV (*.csv)'
            output_type = 'GeoDataFrame'
            EPSG = self.CRS_dict[self.CRS]

        self.outFile = QFileDialog.getSaveFileName(self, 'Salvar DataFrame',
                                                   self.folder, file_options)
        path = self.outFile[0]

        if path != '':
            try:
                if output_type == 'DataFrame' or path.endswith(
                        '.xlsx') or path.endswith('.csv'):
                    ...
                    msg = QMessageBox(parent=self,
                                      text='DataFrame exportado com sucesso!')
                    ...
                    GDF = geopandas.GeoDataFrame(
                        ...
                        geometry=geopandas.points_from_xy(
                            self.DF[self.E_column],
                            self.DF[self.N_column]
                        )
                    )
                    ...
                    if path.endswith('.shp'):
                        ...
                        layerName, ok = QInputDialog.getText(
                            self,
                            'Nome da camada',
                            'Nome para a camada do GeoPackage:',
                            text='layer'
                        )
                        if ok:
                    ...
                    if ok and len(GDF) <= 150:
                        dlg = QMessageBox(QMessageBox.Icon.Question, 'Sucesso',
                                          ...
                                          'Deseja visualizar uma prévia dos '
                                          'resultados?',
                        ...
                        if reply == QMessageBox.StandardButton.Yes:
                            try:
                                new_GDF = (geopandas.read_file(
                                        path, layer=layerName)
                                        if path.endswith('.gpkg')
                                        else geopandas.read_file(path)
                                )
                                ...
                                msg = QMessageBox(parent=self,
                                                  text=f'Não foi possível '
                                                       f'plotar os dados no '
                                                       f'mapa.\n\n%s{str(e)}')
                                msg.setWindowTitle('Erro')
                                msg.setIcon(QMessageBox.Icon.Critical)
                                msg.exec()
                ...
                if str(e) == "ESRI Shapefile does not support datetime fields":
                    msg = QMessageBox(parent=self,
                                      text='O formato shapefile não suporta '
                                           'campos de data e hora. Converta as '
                                           'colunas do tipo "Timestamp" e '
                                           '"Datetime" para "String" e tente '
                                           'novamente.')
                elif str(e) == "Invalid field type <class 'datetime.datetime'>":
                    msg = QMessageBox(parent=self,
                                      text='Há um campo de data e hora com '
                                           'formato inválido no DataFrame. '
                                           'Converta as colunas de data/hora '
                                           'para "Datetime" ou "Timestamp" e '
                                           'tente novamente.')
                else:
                    msg = QMessageBox(parent=self,
                                      text=f'Ocorreu um erro ao exportar os '
                                           f'dados.\n\n{str(e)}')
                msg.setWindowTitle('Erro')
                msg.setIcon(QMessageBox.Icon.Critical)
                msg.exec()
